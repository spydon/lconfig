Config {
    ...
    commands = [
        ...
        Run Com "/bin/bash" ["-c", "~/scripts/so-flame.sh"] "flame" 2000,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader%  }{ %flame% | %bitcoin% | %ethereum% | %eos% | %monero% | %ripple% | %multicpu% | %dynnetwork% | %memory% | %battery% | <fc=#ee9a00>%date% </fc>"
}
